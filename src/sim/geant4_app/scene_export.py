...
DEFAULT_DETECTOR_COINCIDENCE_WINDOW_S = 1.0e-6
...
class ExportedGeant4Source:
    ...
    intensity_cps_1m: float | None = None
    ...
    def __post_init__(self) -> None:
        """Require exactly one finite positive source-strength semantic."""
        supplied = [
            value
            for value in (self.intensity_cps_1m, self.activity_bq)
            if value is not None
        ]
        if len(supplied) != 1:
            raise ValueError(
                "A Geant4 source requires exactly one of intensity_cps_1m "
                "or activity_bq."
            )
        strength = float(supplied[0])
        if not np.isfinite(strength) or strength <= 0.0:
            raise ValueError("Geant4 source strength must be finite and positive.")
        if self.intensity_cps_1m is not None:
            object.__setattr__(self, "intensity_cps_1m", strength)
        else:
            object.__setattr__(self, "activity_bq", strength)
...
class ExportedDetectorModel:
    ...
    coincidence_window_s: float = DEFAULT_DETECTOR_COINCIDENCE_WINDOW_S
    crystal_shape: str = "sphere"
    crystal_material: str = "cebr3"
    housing_material: str = "aluminum"

    def __post_init__(self) -> None:
        """Validate the detector pulse-integration time contract."""
        window_s = float(self.coincidence_window_s)
        if not np.isfinite(window_s) or window_s <= 0.0:
            raise ValueError(
                "Detector coincidence_window_s must be finite and positive."
            )
        object.__setattr__(self, "coincidence_window_s", window_s)
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "coincidence_window_s": float(self.coincidence_window_s),
