...
def _build_command(
    *,
    ...
    preprocessor_defines: tuple[str, ...] = (),
) -> list[str]:
    ...
    return [
        ...
        *profile_flags,
        *pgo_flags,
        *(f"-D{definition}" for definition in preprocessor_defines),
...
def _write_build_metadata(
    *,
    ...
    pgo_mode: str,
    pgo_dir: Path | None,
    trajectory_diagnostic: bool = False,
) -> None:
    ...
    payload = {
        ...
        "trajectory_diagnostic": bool(trajectory_diagnostic),
    }
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="PGO directory; diagnostic builds default to their own output's .pgo directory.",
    )
    parser.add_argument(
        "--trajectory-diagnostic",
        action="store_true",
        help=(
            "Build an opt-in diagnostic executable with native Geant4 "
            "step-trajectory export. Use a separate --output path; this mode "
            "must not replace the independently approved production binary."
        ),
    )
    ...
    if args.trajectory_diagnostic:
        production_path = (ROOT / "build" / "geant4_sidecar").resolve()
        production_paths = {
            production_path,
            production_path.with_name("geant4_sidecar.build.json"),
            production_path.with_name("geant4_sidecar-build.json"),
        }
        metadata_path = (
            None
            if args.metadata_output is None
            else Path(args.metadata_output).expanduser().resolve()
        )
        if output_path in production_paths or metadata_path in production_paths | {
            output_path
        }:
            raise SystemExit(
                "--trajectory-diagnostic requires a separate --output path "
                "and separate build metadata."
            )
    ...
    default_pgo = (
        output_path.with_name(f"{output_path.name}.pgo")
        if args.trajectory_diagnostic
        else ROOT / "build" / "geant4_sidecar.pgo"
    )
    pgo_dir = (
        None
        if args.pgo == "off"
        else Path(args.pgo_dir or default_pgo).expanduser().resolve()
    )
    if args.pgo == "generate":
        assert pgo_dir is not None
        pgo_dir.mkdir(parents=True, exist_ok=True)
    elif args.pgo == "use" and (
        pgo_dir is None or not pgo_dir.is_dir() or not any(pgo_dir.iterdir())
    ):
        raise SystemExit(
    ...
    if args.metadata_output is not None:
        ...
        _write_build_metadata(
            ...
            pgo_mode=args.pgo,
            pgo_dir=pgo_dir,
            trajectory_diagnostic=args.trajectory_diagnostic,
        )
